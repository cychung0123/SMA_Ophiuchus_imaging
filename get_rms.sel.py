...
modelmap = field+'.'+ifband+'.'+sideband+'.'+track+'.10.sel.ch0.model.fits'
residualmap = field+'.'+ifband+'.'+sideband+'.'+track+'.sel.ch0.residual.fits'
dirtymap = field+'.'+ifband+'.'+sideband+'.'+track+'.sel.ch0.dirty.fits'
...
snr=np.max(clean_img)/rms

c = 0
for p in range(4):
    if snr * math.sqrt(p+1) > 5.0:
        c = p+1
        break
    elif p+1 == 4:
        c = p+1

write_to_file('com_'+track+'.sel.txt', field, c)
