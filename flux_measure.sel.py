...
def gaussian_fitting(z, rms):
    ...
    SNR=max_flux/rms
    sep = math.sqrt((hduwcs.wcs_world2pix(cen[0],cen[1],0)[0]-cen_x)**2+(hduwcs.wcs_world2pix(cen[0],cen[1],0)[1]-cen_y)**2)
    ...
    if (max_flux/rms > 3):
        try:
            ...
            if perr[3] > 0.3 or perr[4] > 0.3:
                ...
                if sep > bmaj*6/cdelt2:
                    major_axis = 0.000000000000
                    minor_axis = 0.000000000000
                    total_flux = 0.000000000000
                    cen = (0.0,0.0)
                    SNR = 0.0
                    return max_flux*1000, major_axis, minor_axis, total_flux, cen, SNR, sep
                else:
                    return max_flux*1000, major_axis, minor_axis, total_flux, cen, SNR, sep

            elif sep > bmaj*6/cdelt2:
                major_axis = 0.000000000000
                minor_axis = 0.000000000000
                total_flux = 0.000000000000
                cen = (0.0,0.0)
                ...
                plt.savefig(field+'_'+track+'.'+ifband+'.'+sideband+'.fitting.sel.pdf', format='PDF', transparent=True)
                plt.close(fig)
...
cleanmap = field+'.'+ifband+'.'+sideband+'.'+track+'.sel.ch0.clean.fits'
if_success = False
try:
...
#    box0 = int(box_cen[0] - 7.5/(cdelt2*3600))
#    box1 = int(box_cen[1] - 7.5/(cdelt2*3600))
#    box2 = int(box_cen[0] + 7.5/(cdelt2*3600))
#    box3 = int(box_cen[1] + 7.5/(cdelt2*3600))

    z = clean_img[box1:box3 , box0:box2]
    peak_flux, major_axis, minor_axis, total_flux, cen, SNR, sep = gaussian_fitting(z, rms)
...
write_to_file('axis_FWHM_'+track+'.sel.txt', field, str(major_axis)+' '+str(minor_axis))
write_to_file('flux_'+track+'.sel.txt', field, total_flux)
write_to_file('SNR_'+track+'.sel.txt', field, SNR)
write_to_file('box_'+track+'.sel.txt', field, box)
write_to_file('center_'+track+'.sel.txt', field, '('+str(cen[0])+','+str(cen[1])+')')
write_to_file('separa_'+track+'.sel.txt', field, sep)
