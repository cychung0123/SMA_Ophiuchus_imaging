    ...
    if (max_flux/rms > 3):
        try:
            ...
            if perr[3] > 0.3 or perr[4] > 0.3:
                ...
                plt.savefig(field+'_'+track+'.'+ifband+'.'+sideband+'.fitting.pdf', format='PDF', transparent=True)
                plt.close(fig)
...
cleanmap = field+'.'+ifband+'.'+sideband+'.'+track+'.ch0.clean.fits'
if_success = False
try:
...
write_to_file('separa_'+track+'.txt', field, sep)
